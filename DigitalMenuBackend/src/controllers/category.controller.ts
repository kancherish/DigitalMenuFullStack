...
export const addCategory = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!name || !restaurant_id) {
    res.status(400).json(
      new ErrorResponse(400, "Missing required parameter(s): name, restaurant_id")
    );
    return;
  }
...
export const getCategoriesByRestaurant = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!restaurant_id) {
    res.status(400).json(
      new ErrorResponse(400, "Valid restaurant_id is required in URL param :r_id")
    );
    return;
  }
  ...
  if (!restaurantExists) {
    res.status(404).json(
      new ErrorResponse(404, `Restaurant ${restaurant_id} not found`)
    );
    return;
  }
