...
const errorMiddleware = (
  ...
  try {
    // Prisma: Record not found (e.g. findUniqueOrThrow / findFirstOrThrow)
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      error.code === "P2025"
    ) {
      const message = (error.meta?.cause as string) ?? "Resource not found";
    ...
    // Prisma: Required relation record missing
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      error.code === "P2015"
    ) {
    ...
    // Prisma: Value out of range for column type
    if (
      error instanceof Prisma.PrismaClientKnownRequestError &&
      error.code === "P2006"
    ) {
    ...
    // Prisma: Validation error (wrong type / malformed input)
    ...
    // Prisma: Query engine crashed or timed out
    if (error instanceof Prisma.PrismaClientRustPanicError) {
      const message = "A critical database error occurred.";
      error = new ErrorResponse(500, message);
    }

    // Prisma: Unknown request error (catch-all for P-codes not handled above)
